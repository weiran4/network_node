...
class StructuredFormulaEliminationTests(unittest.TestCase):
    ...
    def test_dependency_stage_plan_keeps_constant_schur_in_ram(self):
        ...
        self.assertNotIn("matrix_register(&W_code);", draft)
        self.assertNotIn("conditionMatrixForCODE(&Gkr_code);", draft)
        self.assertNotIn("conditionMatrixForCODE(&W_code);", draft)
    ...
    def test_dependency_stage_plan_moves_variable_schur_to_code_update(self):
        ...
        self.assertNotIn("MATRIX_ Gkr_code", draft)
        self.assertNotIn("get_CODE(&Gkr_code", draft)
    ...
    def test_constant_g_blocks_precompute_for_dynamic_ihis_and_vk_in_ram(self):
        G1, G2, h = sp.symbols("G1 G2 h")
        nodes = ["A", "B", "X"]
        G = sp.Matrix([[G1, 0, -G1], [0, G2, -G2], [-G1, -G2, G1 + G2]])
        Ihis = sp.Matrix([[0], [0], [h]])
    ...
    def test_constant_g_runtime_inverse_precomputes_matrices_in_ram(self):
        G1, G2, h = sp.symbols("G1 G2 h")
        nodes = ["A", "B", "X"]
        G = sp.Matrix([[G1, 0, -G1], [0, G2, -G2], [-G1, -G2, G1 + G2]])
        Ihis = sp.Matrix([[0], [0], [h]])
        ...
        self.assertNotIn("conditionMatrixForCODE(&Gkr_code);", ready_block)
        self.assertNotIn("conditionMatrixForCODE(&Gkk_code);", ready_block)
        self.assertIn("conditionMatrixForCODE(&W_code);", ready_block)
        self.assertNotIn("set_CODE(&Gkr_code", ready_block)
        self.assertNotIn("set_CODE(&Gkk_code", ready_block)
        self.assertNotIn("set_CODE(&W_code", ready_block)
        self.assertNotIn("matrix_mult_CODE(&tmp_W_Gkr_code, &W_code, &Gkr_code);", ready_block)
        self.assertIn("MATRIX_ Grk_code", draft)
        self.assertNotIn("MATRIX_ Gkr_code", draft)
        ...
        self.assertIn("set(&Grk_code, 0, 0, Grk_A_X);", draft)
        self.assertNotIn("set(&Gkr_code", draft)
        self.assertIn("set(&Gkk_code, 0, 0, Gkk_X_X);", draft)
        self.assertIn("set(&W_code, 0, 0, 1.0 / get(&Gkk_code, 0, 0));", draft)
        self.assertIn("matrix_mult(&tmp_Grk_W_code, &Grk_code, &W_code);", draft)
        self.assertIn("set(&tmp_W_Gkr_code, row, col, get(&tmp_Grk_W_code, col, row));", draft)
        self.assertNotIn("matrix_mult(&tmp_W_Gkr_code, &W_code, &Gkr_code);", draft)
    ...
    def test_c_draft_without_internal_nodes_uses_original_g_and_ihis(self):
        ...
        self.assertIn("varG_A_A = G2;", draft)
        self.assertIn("varG_A_B = -G2;", draft)
    ...
    def test_no_internal_nodes_split_constant_and_dynamic_g_terms(self):
        ...
        self.assertIn("varG_N2_N2 = G;", draft)
        self.assertIn("varG_N2_N4 = -G;", draft)
        self.assertIn("varG_N4_N4 = G;", draft)
        self.assertNotIn("set_CODE(&G_code", draft)
        self.assertNotIn("get_CODE(&G_code", draft)
