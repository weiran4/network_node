...
- Pack multi-case exports can align cases that expose the same external ports but have different eliminated internal nodes. Missing internals are represented by backend-only Gkk placeholders, real recovery work is emitted per case, and generated C aliases use the user's node names such as `inner_left` instead of backend ids.
- Multi-case owner promotion is conservative: if any case for an alias depends on CODE or per-step history, the whole alias is promoted to the highest required lifecycle and a warning is shown. The normal `case_id` is fixed before simulation; only explicitly marked runtime-mutable case groups use their own CODE-stage selector, and those selectors never enter `createGValue` conditions.
